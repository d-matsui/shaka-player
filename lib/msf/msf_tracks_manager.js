...
shaka.msf.TracksManager = class {
  ...
  async handleIncomingStream_(stream) {
    ...
    try {
      ...
      const isSubgroupHeader =
        (streamType >= 0x08 && streamType <= 0x0D) ||
        (streamType >= 0x10 && streamType <= 0x15) ||
        (streamType >= 0x18 && streamType <= 0x1D);
      if (isSubgroupHeader) {
  ...
  async handleSubgroupStream_(reader, streamType) {
    try {
      ...
      // 0x08-0x0D: draft-11 (without EndOfGroup)
      // 0x10-0x15: draft-14 without EndOfGroup
      // 0x18-0x1D: draft-14 with EndOfGroup
      // TODO: When EndOfGroup is set (0x18-0x1D), we could close the
      // stream early after the last object instead of waiting for
      // reader.done().
      let subgroupId = null;
      const hasExtensions =
